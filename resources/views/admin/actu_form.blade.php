    ...
    <!--modif membre bureau-->
        <div class="box-modif-student_word mt-3 mb-4 p-3 button-container bg-white border shadow-sm">
            <form class=" needs-validation p-2" novalidate id="new-member"
                  action="{{ ($update==1) ? url('admin/actu/update/'.$actu->id) :  url('admin/actu/store') }}"
                ...
                <div class="form-row">
                    <div class="col-md-12 mb-2">
                        ...
                        <input type="text" class="form-control" id="title" placeholder="Titre" name="title"
                               value="{{ old('title') ?? ($actu->title ?? '') }}" required>
                                ...
                                @if(count($categories))
                                    @foreach($categories as $category)
                                        <input type="checkbox" id="{{ $category->id }}" name="categories[]"
                                               value="{{ $category->id }}" @php if(isset($actu) && in_array($category->id,$actucats)) echo 'checked="checked"'; @endphp>

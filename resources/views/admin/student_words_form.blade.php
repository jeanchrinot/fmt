...
<main class="col-sm-9 mx-auto col-xs-12 content pt-3 pl-0">
  ...
  <div class="box-modif-student_word mt-3 mb-4 p-3 button-container bg-white border shadow-sm">
    <form class=" needs-validation p-2" novalidate id="new-member" action="{{ ($update==1) ? route('page.item.update',['item'=>'student_words','id'=>$word->id]) : route('page.item.store',['item'=>'student_words']) }}" method="post">
      {{ csrf_field() }}
      ...
      <div class="form-row">
        <div class="col-md-4 mb-2">
          ...
          <select class="form-control" id="user" name="user" required>
            ...
            @foreach($users as $key=>$user)
              <option value="{{ $user->id }}" {{ ($selected_user==$user->id) ? 'selected="selected"' : '' }}>{{ $user->surname }} {{ $user->name }}</option>
            @endforeach
            @elseif($update==1)
             <option value="{{ $users->id }}" {{ ($selected_user==$users->id) ? 'selected="selected"' : '' }}>{{ $users->surname }} {{ $users->name }}</option>
            @endif
        ...
        <div class="col-md-4 mb-2">
          ...
          <textarea rows="5" type="text" class="form-control" name="words" id="words" required>{{ old('words') ?? ($word->words ?? '') }}</textarea>
          <div class="invalid-feedback">

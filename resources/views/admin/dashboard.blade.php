                ...
                <table class="table table-bordered table-striped mt-0 text-center">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="align-middle text-center">{{ date_format($message->created_at,"d/m/Y H:i") }}
                            </td>
                            <th>{{ substr($message->message,0,39) }}{{ strlen($message->message)> 40 ? '...' : '' }}
                            </th>
                            <td class="align-middle">
                                <span id="status{{$message->id}}"
                                    class="message_status badge {{ ($message->viewed) ? 'badge-success':'badge-info' }}">{{ ($message->viewed) ? 'lu':'nouveau' }}
                                </span>
                            </td>
                            <td class=" align-middle text-center d-flex justify-content-center">
                                <button id="{{$message->id}}" class="action btn btn-theme view_message"
                                    data-toggle="modal">
                                    <i class="fa fa-eye"></i>
                                </button>
                                <button id="{{$message->id}}" class="action btn btn-danger delete_message"
                                    data-toggle="modal">
